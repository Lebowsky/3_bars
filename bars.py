...
def get_closest_bar(json_data, longitude, latitude):
    bars_coordinates = [[feature['properties']['Attributes']['Name'], feature['geometry']['coordinates']]
                     for feature in json_data['features']]
    #Найдем расстояния точек
    bars_coordinates_distances = [[bar[0], bar[1],
    ...
    bars_coordinates_min_distances = [bar for bar in bars_coordinates_distances if bar[2] == min_dist]
    return bars_coordinates_min_distances[0][0]


def max_min_seats_count(json_data, func_max_min):
    bars_attributes = [feature['properties']['Attributes'] for feature in json_data['features']]
    # создадим список всех размеров для поиска максимума/минимума
    seats_count = [bar_attributes['SeatsCount'] for bar_attributes in bars_attributes]
    biggest_bars = [bar_attributes['Name'] for bar_attributes in bars_attributes
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        print("Самый близкий бар: " +
              get_closest_bar(json_data, float(sys.argv[2]), float(sys.argv[3])))
